...
const LiveAlerts = () => {
  const [alerts, setAlerts] = useState([]);
  // Use a ref to keep track of alert IDs we've already shown a toast for
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        ...
        // Check for new, high-severity alerts to show as a pop-up
        data.forEach((alert) => {
          if (alert.severity === 'high' && !shownAlertsRef.current.has(alert._id)) {
            // Display a toast notification
            toast.error(alert.message);
            // Add the alert ID to our set so we don't show it again
            shownAlertsRef.current.add(alert._id);
          }
        });
